...
func _parse_ws_packet(raw: PackedByteArray):
	#print("_parse_ws_packet called, size=", raw.size(), " first_byte=", raw[0] if raw.size() > 0 else -1)
	
	if raw.is_empty():
		return

	var packet_type = raw[0] - 48

	match packet_type as EnginePacketType:
		EnginePacketType.MESSAGE:
			var body_bytes = raw.slice(1)
			# Replace NUL bytes (0x00) with underscore (0x5F) at byte level
			# before any string conversion — GDScript strings cannot contain NUL
			for i in range(body_bytes.size()):
				if body_bytes[i] == 0:
					body_bytes[i] = 95  # ASCII '_'
		...
		EnginePacketType.OPEN:
			_on_open(raw.get_string_from_ascii())
		_:
			pass
